...
public class Terrain implements IsoFunction {
    ...
    public function invalidate(region:AABB, space:Space):void {
        ...
        var x0:int = int(region.min.x / cellSize);
        if (x0 < 0) x0 = 0;
        var y0:int = int(region.min.y / cellSize);
        if (y0 < 0) y0 = 0;
        var x1:int = int(region.max.x / cellSize);
        if (x1 >= width) x1 = width - 1;
        var y1:int = int(region.max.y / cellSize);
    ...
    public function iso(x:Number, y:Number):Number {
        ...
        var ix:int = int(x);
        if (ix < 0) ix = 0; else if (ix >= bitmap.width)  ix = bitmap.width - 1;
        var iy:int = int(y);
        if (iy < 0) iy = 0; else if (iy >= bitmap.height) iy = bitmap.height - 1;
        var fx:Number = x - ix;
